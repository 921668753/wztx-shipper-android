...
import com.ruitukeji.zwbh.entity.NationalCity;
    ...
    public void onoptionsSelectListener() {
        ...
        pvOptions = new OptionsPickerView.Builder(context, new OptionsPickerView.OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int option2, int options3, View v) {
                ...
                PreferenceHelper.write(context, StringConstants.FILENAME, "currentLocationProvince", options1Items1);
                PreferenceHelper.write(context, StringConstants.FILENAME, "currentLocationCity", options1Items2);
                PreferenceHelper.write(context, StringConstants.FILENAME, "currentLocationArea", options1Items3);
                getAddress(tx);
            }
        })
                /*.setSubmitText("确定")
    ...
    private void initJsonData(int type) {//解析数据
        try {
            ...
            for (int i = 0; i < jsonBean.size(); i++) {//遍历省份
                ...
                if (currentLocationProvince.equals(jsonBean.get(i).getText())) {
                ...
                for (int c = 0; c < jsonBean.get(i).getChildren().size(); c++) {//遍历该省份的所有城市
                    ...
                    if (currentLocationCity.equals(jsonBean.get(i).getChildren().get(c).getText())) {
                    ...
                    if (jsonBean.get(i).getChildren().get(c).getChildren() == null
                        ...
                        for (int d = 0; d < jsonBean.get(i).getChildren().get(c).getChildren().size(); d++) {//该城市对应地区所有数据
                            ...
                            if (currentLocationArea.startsWith(AreaName)) {
...
//        mHandler.sendEmptyMessage(MSG_LOAD_SUCCESS);
    ...
    public void showLoadingDialog(String title) {
        if (mLoadingDialog1 == null) {
            ...
            mLoadingDialog1.getProgressHelper().setBarColor(context.getResources().getColor(R.color.lonincolors));
